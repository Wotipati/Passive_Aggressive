...
def main():
    train_dataset = SimpleDataset(total_num=800, is_confused=True,  x=3, y=5, seed=1)
    test_dataset  = SimpleDataset(total_num=200, is_confused=False, x=3, y=5, seed=2)
    ...
    fig_left.set_title("Input Data & Training Result", fontsize=15)
    fig_left.tick_params(labelsize=10)
    
    fig_right.set_xlim([0, len(train_dataset.y)])
    fig_right.set_ylim([0,1])
    fig_right.set_title("Accuracy", fontsize=15)
